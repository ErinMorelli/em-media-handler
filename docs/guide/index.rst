...
Check out the :doc:`requirements` for details on required python packages and 3rd party applications.
...
Use any text editor to open and edit the file. Refer to the :doc:`settings` article more details on the settings available.
...
to view the available options. Read more about the :doc:`commandline`.

It is also possible to integrate EM Media Handler with `Deluge <http://deluge-torrent.org/>`_ using the `Execute <http://dev.deluge-torrent.org/wiki/Plugins/Execute>`_ plugin. Read more about :doc:`deluge`.
